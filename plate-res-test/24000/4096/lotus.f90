program swimming_plate
  ...
  ! -- Hyperparameters
    ...
    real, parameter    :: A = 0.1*L, St_d = 0.3, k_x=0., k_z=8.0, h_roughness=0.0
  ...
  real pure function h(x)
    real,intent(in) :: x(3)
    h = amp(x(1))*cos(arg(x(1)))
  end function h
  real pure function doth(x)
    real,intent(in) :: x(3)
    doth = 2*pi*f*amp(x(1))*sin(arg(x(1)))
  end function doth
  pure function dh(x)
    real,intent(in) :: x(3)
    real            :: dh(3)
    dh = 0
    dh(1) = damp(x(1))*cos(arg(x(1))) &
          - amp(x(1))*sin(arg(x(1)))*2*pi/L
  end function dh
  real pure function arg(x)
    real,intent(in) :: x
    arg = 2*pi*(k_coeff*x/L-f*flow%time)
  end function arg
  real pure function amp(x)
    real,intent(in) :: x
    real            :: xp
    xp = min(max(x/L,0.),1.)
    amp = L*(a_coeff*xp**2 + b_coeff*xp + c_coeff)
  end function amp
  real pure function damp(x)
    real,intent(in) :: x
    real            :: xp
    xp = min(max(x/L,0.),1.)
    damp = (2.*a_coeff*xp+b_coeff)
  end function damp
  !
